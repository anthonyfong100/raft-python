...
class TestMessageTypes(unittest.TestCase):
    def test_command_is_valid(self):
        valid_set_command: SetCommand = SetCommand(
            1, {"key": 1, "value": 2}, MID="MID")
        self.assertTrue(valid_set_command.is_valid(),
                        "Set command should have key and value")

        invalid_set_command: SetCommand = SetCommand(
            1, {"value": 2}, MID="MID")
        self.assertFalse(invalid_set_command.is_valid(),
                         "Set command should have key and value")

        valid_get_command: GetCommand = GetCommand(1, {"key": 1}, MID="MID")
        self.assertTrue(valid_get_command.is_valid(),
                        "Get command should have key")

        invalid_get_command: GetCommand = GetCommand(
            1, {"value": 2}, MID="MID")
    ...
    def test_serialize_command(self):
        ...
        expected_serialization: dict = {
            'term_number': 1, 'command_type': 'set', "MID": "MID", 'args': {'key': 1, 'value': 2}}
        self.assertDictEqual(valid_set_command.serialize(),
                             expected_serialization)

        valid_get_command: GetCommand = GetCommand(1, {"key": 1}, MID="MID")
        expected_serialization: dict = {
            'term_number': 1, 'command_type': 'get', "MID": "MID", 'args': {'key': 1}}
    ...
    def test_deserialize_command_function(self):
        ...
        get_command: SetCommand = GetCommand(1, {"key": 1}, MID="MID")
